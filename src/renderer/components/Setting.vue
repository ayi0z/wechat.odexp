                ...
                <el-collapse-item title="清空缓存" name="2">
                    ...
                    <div>
                        <el-button
                            ...
                            @click="onClear('openid')"
                        >清空 OpenID 缓存</el-button>
                        <el-button
                            type="danger"
                            size="mini"
                            icon="el-icon-delete"
                            style="margin: 3px 3px;"
                            @click="onClear('userinfo')"
                        >清空 UserInfo 缓存</el-button>
                        <el-button
                            type="danger"
                            size="mini"
                            icon="el-icon-delete"
                            style="margin: 3px 3px;"
                            @click="onClear('runtime')"
                        >清空 Runtime 缓存</el-button>
                    </div>
                </el-collapse-item>
            </el-collapse>
        </el-drawer>
    ...
    export default {
      ...
      methods: {
        ...
        onClear (key) {
          switch (key) {
            case 'openid':
              this.$db.openid.del({}).then(res => this.$noti.cc(res))
              break
            case 'userinfo':
              this.$db.user.del({}).then(res => this.$noti.cc(res))
              break
            case 'runtime':
              this.$db.runtime.del({}).then(res => this.$noti.cc(res))
              break
          }
        }
      }
